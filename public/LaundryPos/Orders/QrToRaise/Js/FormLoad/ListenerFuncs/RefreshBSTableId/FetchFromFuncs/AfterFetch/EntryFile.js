...
let StartFunc = ({ inDataToShow, inQrCodeData }) => {
    ...
    let jCarLocalIsQrCodeRaisedData = jVarWithQrCodeData.filter(e => e.IsQrCodesRaised !== true);

    if (jCarLocalIsQrCodeRaisedData.length === 0) swal.fire({
        icon: "error",
        text: "No Data"
    });
    
    let LocalArrayReverseData = jCarLocalIsQrCodeRaisedData.slice().reverse();
